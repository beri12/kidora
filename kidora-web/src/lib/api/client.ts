...
async function request<T>(method: string, path: string, body?: unknown, query?: Query): Promise<T> {
  ...
  let isJson = true;
  try { data = text ? JSON.parse(text) : undefined; } catch { data = text; isJson = false; }

  // A 200 that is not JSON is not a payload, and must not be handed to a
  // caller as one. With NEXT_PUBLIC_API_URL unset, BASE is "" and every
  // request goes to this Next app instead of the API: /school/dashboard
  // answers 200 with the page's own HTML, which sailed through as `data` and
  // blew up far away as "Cannot read properties of undefined (reading 'name')"
  // — a white screen naming neither the request nor the cause.
  if (res.ok && text && !isJson) {
    if (typeof window !== "undefined" && !BASE) {
      // eslint-disable-next-line no-console
      console.error(`API request to ${path} was sent to the web app, not the API: NEXT_PUBLIC_API_URL is not set.`);
    }
    throw new ApiError(res.status, "Kidora couldn't read the server's reply. Try again in a moment.", text.slice(0, 200));
  }

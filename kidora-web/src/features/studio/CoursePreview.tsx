...
import { Award, BookOpen, Check, ChevronLeft, Clock, FileCheck2, Lock, Star, Users } from "lucide-react";
...
function Preview({ courseId }: { courseId: string }) {
  ...
  return (
    ...
    >
      {q.isPending ? <Skeleton className="h-[70vh]" /> : q.isError ? <ErrorState error={q.error} retry={() => q.refetch()} /> : (() => {
        const p = q.data!;

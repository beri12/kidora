/**
 * Every country / territory that can receive an SMS, with its ISO 3166-1
 * alpha-2 code and international dial code.
 *
 * Flags are NOT stored here. They are derived from the ISO code at render
 * time (see `flagEmoji`), which keeps this table half the size and means a
 * flag can never drift out of sync with its country.
 */
...
export interface Country {
  ...
  iso2: string;
  name: string;
  /** International dial code without the "+", e.g. "251". */
  dial: string;
}

/**
 * Turns "ET" into 🇪🇹 by mapping each letter onto its regional indicator
 * symbol. Every platform with emoji support renders the pair as a flag, so
 * there are no flag images to ship or to fail to load.
 */
...
/** Pinned to the top of the picker before the full A–Z list. */
export const POPULAR_ISO2 = ['ET', 'US', 'GB', 'CA', 'NG', 'KE', 'ZA', 'IN', 'AE', 'DE'];

/** The country selected when we cannot infer one from the browser. */
export const DEFAULT_ISO2 = 'ET';
 ...
 * ("am-ET" → ET). Falls back to DEFAULT_ISO2.
 */
export function guessCountry(): Country {
  if (typeof navigator !== 'undefined') {
    ...
    for (const tag of tags) {
      ...
      const hit = region && findCountry(region);
      if (hit) return hit;
    }
  }
  return findCountry(DEFAULT_ISO2)!;
}

/**
 * Case- and accent-insensitive search over country name, ISO code and dial
 * code, so "+251", "251", "eth" and "Ethiopie"-style typing all land on the
 * right row. Results keep the list's own ordering.
 */
...
/**
 * Joins a dial code and the digits the user typed into an E.164 number.
 * Leading zeros are dropped: people write "0911 22 33 44" for a number that
 * is "+251911223344" internationally, and typing the trunk prefix is the
 * single most common way to get an OTP silently delivered nowhere.
 */
...
/** E.164 allows 15 digits in total, and no real number is shorter than 7. */
export function isValidE164(value: string): boolean {
  return /^\+[1-9]\d{6,14}$/.test(value);
}

/** Groups the national digits in 3s purely for readability while typing. */

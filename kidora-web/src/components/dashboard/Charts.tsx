  ...
  return (
    <ResponsiveContainer width="100%" height={height}>
      <LineChart data={rows} margin={{ top: 8, right: 8, left: -18, bottom: 0 }}>
        ...
        <YAxis tickLine={false} axisLine={false} tick={{ fontSize: 11, fill: "#6B7280" }} domain={percent ? [0, 100] : ["auto", "auto"]} tickFormatter={(v) => (percent ? `${v}%` : String(v))} />
        <Tooltip contentStyle={{ borderRadius: 12, border: "1px solid #EEF0F4", fontSize: 12 }} formatter={(v) => (percent ? `${v}%` : String(v))} />
  ...
  return (
    <ResponsiveContainer width="100%" height={height}>
      <BarChart data={rows} margin={{ top: 18, right: 4, left: -22, bottom: 0 }}>
        ...
        <Tooltip cursor={{ fill: "#F7F5FF" }} contentStyle={{ borderRadius: 12, border: "1px solid #EEF0F4", fontSize: 12 }} formatter={(v) => [`${v} min`, "Learning"]} />
        <Bar dataKey="min" fill={color} radius={[6, 6, 0, 0]} maxBarSize={28} isAnimationActive={false}>
          <LabelList dataKey="min" position="top" formatter={(v) => (v ? `${v} min` : "")} style={{ fontSize: 10, fill: "#6B7280" }} />
        </Bar>
      </BarChart>
    </ResponsiveContainer>
  );
